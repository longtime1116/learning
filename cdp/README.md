...
- [x] Weighted Transitionパターン（重みづけラウンドロビンDNSを使った移行）
  - AWS Route53(名前解決サービス)の重み付けラウンドロビン機能を使う
  - Route53 のホステッドゾーンに既存システムのレコードセットを作成する(サービス群をセットする)
  - 別のリージョンに同じ構成で用意する。
  - 新システムへの振り分けを最初は1%など少ない比率から初めて、その比率を高めていくことで最終的には完全に移行できる。
  - 注意点として、DBが別々だと問題が起きるかもしれないのでケアする必要がある。
- [x] Log Aggregation パターン(ログの集約)
  - サーバの台数が多いシステムでは、アクセスログなどが各サーバに分散してしまう、集計や分析に手間がかかってしまう。
  - ログ保存用の S3 のバケットを用意して、そこに転送するようにしておくことでその課題を解決できる
  - S3と親和性の高いEMRやRedshiftを使って、容易にログ解析を実行できる。
- [x] Ondemand Activationパターン（メンテナンス時の一時的な設定変更）
  - 踏み台サーバ/NATサーバ用の AMI を用意して、必要な時のみその EC2 を起動して処理をする。
  - また、一時的に許可するセキュリティグループを用意して、必要な時のみ必要なインスタンスにそのグループを適用する。
  - こうすることで、セキュリティを高めつつメンテナンス性をあげることができる。
  - 踏み台サーバはシステムへのアクセス(ログイン)が必要になった時、NAT サーバは外部への通信が必要になった時(ソフトウェアアップデートなど)
  - オペレーションミスが起こらないように、CloudFormation でテンプレート化すると安全である。
## ネットワークのパターン
- [x] Backnetパターン（管理用ネットワークの設置）
  - ENI（仮想ネットワークインタフェース）というものがある。
  - 基本的に EC2 にはデフォルトで 1 つの ENI が割り当てられていて、この ENI と IP アドレスが紐づいている。
  - EC2 に二つの ENI を割り当て、一つは社外向け、port 80 を開き、もう一つは社内向け、port 22 を開く(ssh) と、セキュリティ強化につながる。
...
- [x] WAF Proxyパターン（高価なWeb Application Firewallの効率的な活用）
...
- [x] Sorry Pageパターン(バックアップサイトへの自動切り替え)
  - 障害などが起きた時に、静的サイトに飛ばしたい。
  - DNS サービスである Route 53 のフェイルオーバー機能と、S3 のウェブサイトホスティング機能を組み合わせる。
  - バックアップサイト用コンテンツを S3 にアップロードする。
  - S3 で静的ウェブホスティング機能を用いて、外部公開するように設定する。
  - Route53 で、メインサイトをプライマリのフェイルオーバーリソースセットとして登録し、S3 をセカンダリのフェイルオーバーリソースセットとして登録する。
  - Route53 のヘルスチェック機能を用いて、メインサイトを監視する。
- [x] Self Registrationパターン（自分の情報をデータベースに自動登録）
  - EC2 起動時に自分のメタデータ(IPアドレス等)を KVS に保存し、停止時には削除する。
  - 適当なタイミングでKVSの登録情報を参照して、モニタリングやバックアップの設定をアップデートする。
...
- [x] Shared Serviceパターン(システム共通サービスの共用化)
  - VPC peering を使うと、異なる VPC 間を接続することができる。
    - これは異なる AWS アカウント、異なるリージョンでも接続可能
    - 1:n の接続設定が可能。
  - 共通するサービスをひとつの VPC 内に設置し、これをハブとして VPC peering で複数の VPC をつなげる。
    - 共通サービスを一元化することで、コストの削減と運用負荷の低減を実現できる
    - 共通サービスだけ異なるAWSアカウントで運用でき、担当部門や企業を分けたり、コストを明確に分けられる
- [x] High Availability NATパターン(冗長化されたNATインスタンス)
  - プライベートサブネットに配置したサーバから外部へのアウトバウンド通信を行う際は、NAT サーバやプロキシサーバを経由する。
  - このとき、NAT サーバやプロキシサーバが単一障害点となってしまうのを防ぐため、冗長化したい。
  - NAT サーバについては、
    - EC2 は通常、トラフィックの送信元と送信先がチェックされるので、自分宛でない通信を受診することはできないが、そのチェックを外すことができる。
    - サブネットのルーティング先としてチェックを外したインスタンスを指定して、NAT 機能を持つ EC2 インスタンスを構築できる。以下手順。
    - パブリックサブネットに、NAT インスタンス用 AMI を元に EC2 インスタンスを複数起動する。
    - NAT インスタンスの"Source/Dest Check" を無効にし、Elastic IPもしくはPublic IPを付与する。
    - プライベートサブネットの 0.0.0.0/0 のルーティングのターゲットとして、NAT インスタンスのうちの 1 つを設定する。
    - アクティブな NAT インスタンスに障害が発生したら、API を使用して、正常な他の NAT インスタンスに切り替える。
  - プロキシサーバの場合は、ELB を用いて、
    - HTTP プロキシサーバを internal ELB 配下に配置する。プライベートサブネットに配置されたインスタンスは internal ELB を HTTP プロキシとして使用し、冗長化されたプロキシサーバ経由でアウトバウンド通信を行う。
    - アプリケーションレベルでのプロキシ設定が必要になる。が、NAT 切り替えのように API を叩いて切り替える必要がないので、よりシンプルに冗長性を確保できる。
    - ELB のヘルスチェック機能と Auto Scaling を使えば、プロキシサーバの自動復旧が可能。
    - 以下手順
      - 複数のパブリックサブネットを用意し、それぞれに EC2 を配置する。Squid, ExaProxy といった HTTP プロキシをインストールする。
      - internal ELB を起動し、プロキシサーバをロードバランシング対象として登録し、ヘルスチェックを有効にする。
      - プライベートサブネットからアウトバウンド通信を行う各インスタンスに対して、HTTP プロキシとして ELB の FQDN を設定する。
